 ...
 * 用于UI显示
 */
data class StringHabitDisplayModel(
    val name: String,
    ...
    val chinesePercentage: Int,
    val lastUsed: Long
) {
    companion object {
        /**
         * 从StringHabitData创建显示模型
         */
        fun from(habit: StringHabitData): StringHabitDisplayModel {
            val total = habit.totalCount
            return StringHabitDisplayModel(
                name = habit.name,
                preferredMethod = habit.preferredMethod,
                englishCount = habit.englishCount,
                chineseCount = habit.chineseCount,
                totalCount = total,
                englishPercentage = if (total > 0) (habit.englishCount * 100 / total) else 0,
                chinesePercentage = if (total > 0) (habit.chineseCount * 100 / total) else 0,
                lastUsed = habit.lastUpdated
            )
        }
    }
    ...
    fun getUsageText(): String {
        return "EN: $englishCount ($englishPercentage%) / CN: $chineseCount ($chinesePercentage%)"
    }
}
